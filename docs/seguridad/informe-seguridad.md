...
La decisión de usar **tres herramientas en lugar de una** no es redundancia: cada
una consulta bases de datos distintas y observa un artefacto distinto. El
resultado de este análisis lo demuestra de forma concreta: `npm audit` reportó
**cero vulnerabilidades**, mientras Trivy encontró **seis de severidad alta** en
la misma aplicación. Ninguna de las dos herramientas se equivocó; simplemente
miraban objetos diferentes.
...
esta aplicación el riesgo es bajo —no hay base de datos ni renderizado de HTML
desde entrada de usuario—, pero en un proyecto con esas características habría
...
El resultado más instructivo del análisis no fue ninguna de las seis
vulnerabilidades encontradas, sino **dónde estaban**: ninguna en el código
escrito, ninguna en las dependencias elegidas, todas heredadas de la imagen base
y de herramientas que ni siquiera se usan en ejecución. Refuerza una idea central
de DevSecOps: la seguridad de una aplicación contenerizada no se decide solo al
escribir código, sino también al elegir qué se empaqueta junto a él.
